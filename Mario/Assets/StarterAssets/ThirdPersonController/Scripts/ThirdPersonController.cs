...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        {
            _canOnlyMoveForward = transform.position.z < _lastZForwardPosition - 10;
            
            if(transform.position.z > _mainCamera.transform.position.z)
            {
                _mainCamera.transform.position = new Vector3(_mainCamera.transform.position.x,
                    _mainCamera.transform.position.y, transform.position.z);
                if (!_canOnlyMoveForward && transform.position.z > _lastZForwardPosition)
                    _lastZForwardPosition = transform.position.z;
            }
            else if (transform.position.z < _mainCamera.transform.position.z - 4 &&
                     !_canOnlyMoveForward)
            {
                _mainCamera.transform.position = new Vector3(_mainCamera.transform.position.x,
